...
import { updateAchievement } from '../../../Request/PutRequests'
import { getAchievement } from '../../../Request/GetRequests'
...
const EditAchievement: React.FC<IEditAchievement> = (props) => {
  const { classes, cx } = useStyles()

  const getData = async (isMounted: boolean) => {
    const res = await getAchievement(props.achievementId)
    if (isMounted) {
      setAchievementData(
        formatAchievement(
          res.name,
          `/achIcons/${res.iconUrl}`,
          res.description,
          res.type,
          res.testScore,
          res.courseId ?? commonCourseId,
          res.count,
        )
      )
    }
  ...
  const picPlaceholder = 'NotSet.png'
      ...
      <Card className={classes.spaces} variant="outlined">
        <Grid container justifyContent="center">
          <CardMedia
            ...
            image={achievementData.picFile ? achievementData.picFile : `achIcons/${picPlaceholder}`}
